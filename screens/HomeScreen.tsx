...
interface HomeScreenProps {
  ...
  wishlist: string[];
  user: any; // Add user prop to check ownership
}
...
const HomeScreen: React.FC<HomeScreenProps> = ({ onMenuClick, onToggleWishlist, wishlist, user }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Check owned tools if user is logged in
        if (user) {
  ...
  return (
    <main className="pb-32 overflow-y-auto no-scrollbar pt-2 md:pt-6 animate-in fade-in duration-500">
      <header className="sticky top-0 z-40 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md px-4 py-3 md:py-5 flex items-center justify-between rounded-2xl mx-2 md:mx-0 shadow-sm border border-transparent dark:border-white/5">
          ...
          <div className="flex overflow-x-auto gap-3 no-scrollbar pb-1 snap-x">
            {banners.map((banner, idx) => (
              <div 
                ...
                className="snap-start min-w-[85%] md:min-w-[300px] h-36 md:h-48 rounded-2xl overflow-hidden bg-zinc-200 dark:bg-zinc-800 shadow-sm cursor-pointer hover:shadow-md transition-all border border-black/5 dark:border-white/5"
              >
      ...
      <section className="mt-8 md:mt-12">
        ...
        <div className="flex overflow-x-auto px-4 gap-3 md:gap-6 no-scrollbar pb-4">
          {digitalTools.map(product => {
             const isOwned = ownedProductIds.includes(product.id);
             return (
            <div key={product.id} className="min-w-[150px] md:min-w-[200px] bg-white dark:bg-zinc-900 p-4 rounded-[2rem] shadow-sm border border-black/5 dark:border-white/5 flex flex-col items-center text-center group cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all" onClick={() => handleProductClick(product)}>
              ...
              {isOwned ? (
                 <button className="w-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 py-2 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest shadow-none border border-green-200 dark:border-green-800 flex items-center justify-center gap-1">
        ...
        <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 md:gap-4">
          {categories.map(cat => (
            <div key={cat.id} className="flex flex-col items-center gap-1.5 cursor-pointer group" onClick={() => navigate(`/search?category=${cat.slug}`)}>
              <div className="w-full aspect-square rounded-2xl bg-white dark:bg-zinc-900 flex items-center justify-center border border-black/5 group-hover:bg-primary group-hover:border-primary transition-all shadow-sm">
                <span className="material-symbols-outlined text-xl md:text-3xl group-hover:text-background-dark transition-colors">{cat.icon}</span>
              </div>
              <span className="text-[10px] md:text-xs font-bold truncate group-hover:text-primary transition-colors">{cat.name}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-10 md:mt-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-6 px-4">
          {physicalProducts.map(product => {
            ...
            return (
              <div key={product.id} className="bg-white dark:bg-zinc-900 rounded-[2rem] p-3 md:p-4 shadow-sm border border-black/5 cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all group relative" onClick={() => navigate(`/product/${product.id}`)}>
                <div className="relative w-full aspect-square bg-zinc-50 dark:bg-zinc-800 rounded-2xl mb-4 overflow-hidden">
                  ...
                  <button 
                    ...
                    className="absolute top-2 right-2 size-8 bg-white/80 dark:bg-black/40 backdrop-blur rounded-full flex items-center justify-center hover:scale-110 active:scale-90 transition-all z-10"
                  >
                     <span className={`material-symbols-outlined text-lg ${isLiked ? 'text-red-500 material-symbols-fill' : 'text-zinc-400'}`}>favorite</span>
                  </button>
                  
                  {product.discount && <div className="absolute top-2 left-2 bg-primary text-background-dark text-[10px] font-black px-2 py-1 rounded-lg">{product.discount}</div>}
                </div>
                ...
                <div className="flex items-center justify-between mt-auto">
                  ...
                  <button className="size-8 md:size-10 bg-zinc-900 dark:bg-primary text-white dark:text-background-dark rounded-xl flex items-center justify-center shadow-md active:scale-95 transition-transform">
